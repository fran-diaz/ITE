...
class mysql implements dbInterface {
    ...
    public $container;
    public $persistent_connections = false;
    ...
    public function bd_connect() {
        if($this->persistent_connections){$this->descriptor = new mysqli($this->dbserver, $this->dbuser, $this->dbpassword, $this->db);}
        else{$this->descriptor = new mysqli($this->dbserver, $this->dbuser, $this->dbpassword, $this->db);}
    ...
    protected function multi_bd($query) {
        $this->result = $this->descriptor->multi_query($query);
        
        if ($this->result === false) {
            if ($this->container->__debug()) {
                $this->container->debug->error('Error '.$this->descriptor->errno.'-'.$this->descriptor->error . " [-->] " . $query);
            } else {
                trigger_error('Error '.$this->descriptor->errno.'-'.$this->descriptor->error . " [-->] " . $query, E_USER_ERROR);
            }
            return false;
        }else{
            return $this->bd_format();
        }
    }
    ...
    protected function bd_format($sum = false) {
        ...
        if ($this->result === true) {
            //$formated[] = true;
            do {
                    $result = $this->descriptor->store_result();
                    if ($result !== false) {
                    ...
                    if ($this->descriptor->more_results() === false) {
                        break;
                    }
                }while ($this->descriptor->next_result());
        } else {     
            if ($this->result !== false) {
     ...
     */
    public function free_query($query, $sum = true,$semicolon_ptr = '##SEMICOLON##') {
        ...
        if (count($queries) > 1) {
            ...
            /*foreach ($queries as $q) {
                if ($this->bd(str_replace('##SEMICOLON##',';',$q)) === false) {
                    $this->container->__error("Subquery of Free query failed: $q");
                }
            }
            if ($this->result !== false) {
            ...
            $result = $this->bd(str_replace('##SEMICOLON##',';',$query));
            if ($result !== false) {
                if(is_array($result)){
                    return $result;
                }else{
    ...
    public function varchar_field($name, $label, $size = 255, $value = "", $config = false, $html_type = 'form') {
        ...
        if(isset($config['table'])){
            ...
            if($list_values){
                if(count($list_values) == 1 && $value == ""){
                    $value = $list_values[0][$name];
                }else{
                    $dl_id = $name.'-'.rand(0, 9999999999);
                    $html .= '<datalist id="'.$dl_id.'">';
                    foreach($list_values as $v){
    ...
    public function datetime_field($name, $label, $value = "", $config = false) {
        $html = '<div class="form-group">';
        $html .= $this->field_label($name, (isset($config['label'])) ? $config['label'] : $label, $config);
        $html .= '<input type="datetime" class="datetimepicker ' . ((isset($config['class'])) ? $config['class'] : "") . '" name="' . $name . '" id="' . $name . '" value="' . $value . '" placeholder="' . ((isset($config['placeholder'])) ? $config['placeholder'] : '') . '" ' . ((isset($config['disabled'])) ? 'disabled="' . $config['disabled'] . '"' : '') . ' />';
        $html .= '</div>';
    ...
    public function id_field($name, $label, $values, $size = "1", $config = false) {
        $new_btn = '<span class="btn_inline_new_id mdi mdi-plus" data-d="'.$this->container->funcs->encode(['t' => $config['external_table']]).'"></span>';
        $html = '<div class="form-group">';
        $html .= $this->field_label($name, (isset($config['label'])) ? $config['label'].$new_btn : $label.$new_btn, $config);
        if (!isset($values) || count($values) <= 0) {
            $html .= '<select name="' . $name . '" id="' . $name . '" disabled="disabled" size="' . ((isset($config['size'])) ? $config['size'] : $size) . '" ' . ((isset($config['class'])) ? 'class="' . $config['class'] . '"' : "") . '  data-d="'.$this->container->funcs->encode(['t' => $config['external_table']]).'">';
            $html .= '<option value="0">Sin datos, deben crearse antes de este paso.</option>';
            $html .= '</select>';
        } else {
            if (isset($config['linked_field'])) {
                $html .= '<select name="' . $name . '" id="' . $name . '" size="' . ((isset($config['size'])) ? $config['size'] : $size) . '" ' . ((isset($config['class'])) ? 'class="' . $config['class'] . '"' : "") . ' data-linked-field="' . $config['linked_field'] . '"' . ((isset($config['disabled'])) ? 'disabled="' . $config['disabled'] . '"' : '') . ' data-d="'.$this->container->funcs->encode(['t' => $config['external_table']]).'">';
            } else {
                $html .= '<select name="' . $name . '" id="' . $name . '" size="' . ((isset($config['size'])) ? $config['size'] : $size) . '" ' . ((isset($config['class'])) ? 'class="' . $config['class'] . '"' : "") . '' . ((isset($config['disabled'])) ? 'disabled="' . $config['disabled'] . '"' : '') . ' data-d="'.$this->container->funcs->encode(['t' => $config['external_table']]).'">';
            }
     ...
     *  - form: if defined as 'no', the field will not be displayed in the add or edit forms. If 'add', only showed in add forms; same with 'edit'.
    ...
    public function fieldConfigTypes() {
        $types = [
            ...
            'data-d-table' => ['description' => 'for cell action data'],
...
SELECT * 
    ...
    public function create_add_form_fields($table, $exclude_fields, $option_0 = false) {
        ...
        foreach ($fields as $field) {
            if (isset($this->fields_config[$field['Field']]['form']) && ($this->fields_config[$field['Field']]['form'] === 'no' || $this->fields_config[$field['Field']]['form'] === 'edit')) {
                continue;
            }
            $name = $field['Field'];
            $auxname = $table . "_id";
            ...
            if (substr($name, -3) === "_id" && (!isset($field_config['external_table']) || $field_config['external_table'] !== 'none')) {
                ...
                if (isset($field_config['external_table'])) {
                    ...
                    $external_rows = $this->select($external_table, "", $order_query, false, false);
                ...
                $field_config['external_table'] = $external_table;
                $field_config['external_id_field'] = $field_name;
            ...
            switch ($type[0]) {
                case "varchar":$field_config['table'] = $table;$html .= $this->varchar_field($name, false, $size, $value, $field_config);
                    break;
    ...
    public function create_edit_form_fields($table, $id, $exclude_fields, $option_0 = false) {
        ...
        foreach ($fields as $field) {
            if (isset($this->fields_config[$field['Field']]['form']) && ($this->fields_config[$field['Field']]['form'] === 'no' || $this->fields_config[$field['Field']]['form'] === 'add')) {
                continue;
            }
            $name = $field['Field'];
            $auxname = $table . "_id";
            ...
            if (strpos($name, "_id") && $field_config['external_table'] !== 'none') {
                ...
                $field_config['external_table'] = $aux_table;
                $field_config['external_id_field'] = $field_name;    
                $html .= $this->id_field($name, false, $values, "1", $field_config);
                unset($row_aux, $values);
                continue;
            ...
            switch ($type[0]) {
                case "varchar":$field_config['table'] = $table;$html .= $this->varchar_field($name, false, $size, $edit_data[0][$name], $field_config);
                    break;
    ...
    public function create_table($table, $table_id_field = null, $external_config = null) {
        ...
        if(isset($external_config['multiSel']) && $external_config['multiSel'] == true){
            $html .= '<th data-sorter="false"><span class="mdi mdi-check-all table__multiSel-check-all" title="'.$this->container->lang->gt('Seleccionar todos').'"></span></th>';
        }
        ...
        if(isset($_SESSION['temp_config'][$table]['page'])){
            if($_SESSION['temp_config'][$table]['page'] === 0){
                $limit_subquery = '0,18446744073709551615';
            }else{
        ...
        if ($rows) {
            ...
            foreach ($rows as $row_num => $row) {
                ...
                if(isset($external_config['notes']) && $external_config['notes'] == true){
                    ...
                    $html .= "<td class=\"table__row-notes\"><span class=\"mdi mdi-comment-processing-outline table__notes\" title=\"".$this->container->lang->gt('Notas')."\"></span>";
                    $html .= ($external_config['notes'] === 'icon_num')?"<span class=\"table__notes-num\">$notes_count</span></td>":'';
                }
                
                foreach ($this->columns[$table] as $field => $label) {
                    if(isset($this->tables_config[$table][$field]['config']['data-d-table'])){
                        $cell_action = (isset($this->tables_config[$table][$field]['config']['cell_action']) && $this->tables_config[$table][$field]['config']['cell_action'] !== 'none') ? 'data-d="'.$this->container->funcs->encode(array('t' => $this->tables_config[$table][$field]['config']['data-d-table'], 'i' => $i)).'" data-action="' . $this->tables_config[$table][$field]['config']['cell_action'] . '" ' : '';
                    }else{
                        $cell_action = (isset($this->tables_config[$table][$field]['config']['cell_action']) && $this->tables_config[$table][$field]['config']['cell_action'] !== 'none') ? ' data-action="' . $this->tables_config[$table][$field]['config']['cell_action'] . '" ' : '';
                    }
                    
                    if($this->tables_config[$table][$field]['type'] == 'int' && substr($field, -3) !== "_id"){
                        $html .= '<td '.$cell_action.' contenteditable="true" data-editable="int" data-d="'.$this->container->funcs->encode(array('t' => $table, 'i' => $i, 'f' => $field)).'">';
                    }elseif($this->tables_config[$table][$field]['type'] == 'int' && substr($field, -3) == "_id" && substr($field, 0, -3) !== $table){
                        $html .= '<td '.$cell_action.' data-editable="id" data-v="'.$row[$field].'" data-d="'.$this->container->funcs->encode(array('t' => $table, 'i' => $i, 'f' => $field)).'">';
                    }elseif($this->tables_config[$table][$field]['type'] == 'varchar'){
                                ...
                                if(preg_match_all('/##([A-Za-z0-9_ñÑ _\-\.\(\)\[\]\'\=\>\,\"]*)##/', $external_config['cols-config'][$field]['cell_template'], $keys)){
                                    ...
                                    foreach($keys[1] as $key){
                                        if(isset($row[$key])){
                                            ...
                                            $aux_key = explode('.',$key);
                                            if(!isset($this->tables_config[$aux_key[0]])){$this->get_table_config ($aux_key[0]);}
                                            $pri_field = array_keys($this->tables_config[$aux_key[0]])[0];
                                            $aux_skey = $this->select($aux_key[0],$pri_field." = '$i'");
                                            if($aux_skey){
                                                $search[] = '##'.$key.'##';
                                                $replace[] = $aux_skey[0][$aux_key[1]]; 
                                            }else{
                                                $search[] = '##'.$key.'##';
                                                $replace[] = "''"; 
                                            }
    ...
    public function get_table_config($table, $external_config = null) {
        ...
        // Columnas virtuales
        if(!is_null($external_config) && is_array($external_config['cols-config'])){
