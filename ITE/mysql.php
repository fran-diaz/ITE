...
class mysql implements dbInterface{
    ...
    protected function bd_format($sum = false) {
        ...
        if(isset($formated)) {
            if($sum) {$formated['numero'] = count($formated);$formated['num_elements'] = $formated['numero'];}
            return $formated;
        }else{
            return false;
        }
    ...
    public function field_label($name,$label = false, $config = false){
        ...
        $html = '<label for="'.$name.'">';
        if(defined(LANG_IN_USE) && LANG_IN_USE !== false){$html .= $this->container->lang->gt($format_name);}
        else{$html .= $format_name;}
    ...
    public function create_add_form_fields($table,$exclude_fields,$option_0 = false){
        ...
        foreach($fields as $field){
            ...
            if(strpos($name, "_id") && $field_config['external_table'] !== 'none'){
                ...
                if(!isset($external_rows[0])){$this->container->__warn("No es posible crear los valores del campo SELECT: no se han recuperado datos de la tabla externa (".substr($name, 0, strlen($name)-3).") [".__FUNCTION__.",".__LINE__."]");}
                else{
            ...
            switch($type[0]){
                case "varchar":$html .= $this->varchar_field($name,false, $size,$value,$field_config);break;
                case "int":$html .= $this->int_field($name,false, $size,$value,$field_config);break;
                case "double":$html .= $this->double_field($name,false, $size,$value,$field_config);break;
                case "date":$html .= $this->date_field($name,false,$value,$field_config);break;
                case "datetime":$html .= $this->datetime_field($name,false,$value,$field_config);break;
                case "time":$html .= $this->time_field($name,false,$value,$field_config);break;
                case "text":$html .= $this->text_field($name,false,$value,$field_config);break;
                case "enum":
                    $values = $size;
    ...
    private function create_external_field_order_query($external_field_config){
    ...
    public function create_edit_form_fields($table,$id,$exclude_fields,$option_0 = false){
        ...
        foreach($fields as $field){
            ...
            if(count($type) >> 1){$size = explode(")", $type[1]);$size = $size[0];}
            switch($type[0]){
                case "varchar":$html .= $this->varchar_field($name,false, $size,$edit_data[0][$name],$field_config);break;
                case "int":$html .= $this->int_field($name,false, $size,$edit_data[0][$name],$field_config);break;
                case "double":$html .= $this->double_field($name,false, $size,$edit_data[0][$name],$field_config);break;
                case "datetime":$html .= $this->datetime_field($name,false,$edit_data[0][$name],$field_config);break;
                case "date":$html .= $this->date_field($name,false,$edit_data[0][$name],$field_config);break;
                case "time":$html .= $this->time_field($name,false,$edit_data[0][$name],$field_config);break;
                case "text":$html .= $this->text_field($name,false,$edit_data[0][$name],$field_config);break;
                case "enum":
                    $values = $size;
                    $tokens = explode(",",$values);
