...
class mysql implements dbInterface{
    ...
    public function bd_connect() {
        $this->descriptor = mysql_connect($this->dbserver, $this->dbuser, $this->dbpassword,true);
        if($this->descriptor === false){
            if($this->container->__debug()){$this->container->debug->error("No ha sido posible conectar al servidor de base de datos:".mysql_error());}
            else{trigger_error("No ha sido posible conectar al servidor de base de datos:".mysql_error(),E_USER_ERROR);}
        }
        mysql_query("SET NAMES 'utf8'");
        if(!mysql_select_db($this->db,$this->descriptor)){
            if($this->container->__debug()){$this->container->debug->error("No ha sido posible seleccionar la base de datos");}
            else{trigger_error("No ha sido posible seleccionar la base de datos",E_USER_ERROR);}
        }
    }
     ...
     * @return integer|boolean Returns autoincrement number (ID) generated by the query, 0 if the query does not generate any new registry or false if fails.
     */
    private function bd($query) {
        $this->result = mysql_query($query,$this->descriptor);
        if($this->result === false){
            if($this->container->__debug()){$this->container->debug->error(mysql_error($this->descriptor)." [-->] ".$query);}
            else{trigger_error(mysql_error($this->descriptor)." [-->] ".$query,E_USER_ERROR);}
        }
     ...
     */
    private function bd_format($sum = false) {
        while($aux = mysql_fetch_array($this->result, MYSQL_ASSOC)) {
            $keys = array_keys($aux);
            $formated[] = $aux;
        }
        if(isset($formated)) {
     ...
     * @param string $sum If the number of elements (num_elements) variable is returned
     ...
     * @return integer ID of generated row in table (autoincrement column)
     */
    ...
    public function delete($table,$id,$skip_log = false) {
        if($skip_log == false){
            ...
            $this -> log($table, $id,"borrar","Valor previo [".mysql_real_escape_string(implode(", ",$data[0]))."]");
        }
